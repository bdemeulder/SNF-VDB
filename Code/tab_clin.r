tab_clin <- function(clinical,groups,contrast=F){
  clinvar <- c()
  mean <- c()
  pvalanova <- c()
  pvalkruskal <- c()
...
#     table1 <- data.frame(clinvar,contrasts,mean,signif(pvalshap,3),signif(pvalkruskal,3),pvalanova)
    ...
    table1 <- data.frame(clinvar,mean,signif(pvalshap,3),signif(pvalkruskal,3),pvalanova)
    names(table1) <- c("Variables",paste(levels(groups),'(n=',summary(groups),')'),"pvalshapiro","pvalkruskal","pvalanova")
  }
